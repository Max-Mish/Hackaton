...
def get_from_table(bd_connection):
    try:
        with bd_connection.cursor() as cursor:
            ...
            cursor.execute(
                f"""SELECT first_name, last_name, middle_name, birth_dt, card_num, driver_license, driver_valid_to, update_dt FROM drivers""")
            ...
            with bd_connection.cursor() as cursor:
                ...
                if cursor.fetchone()[0]:
                    cursor.execute(
                        ...
                        """,
                        (row[7], row[1], row[0], row[2], row[3], row[4], row[5], row[6], True))
                else:
